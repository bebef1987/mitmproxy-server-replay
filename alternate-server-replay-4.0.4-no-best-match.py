    ...
    def next_flow(self, request):
        ...
        if hsh in self.flowmap:
            if ctx.options.server_replay_nopop:
                if (len(self.flowmap[hsh]) >  1):
                    if (self.flowmap[hsh][0].request.url == "https://en.wikipedia.org/w/load.php?debug=false&lang=en&modules=startup&only=scripts&skin=vector"):
                        print("")
